      ...
      let now_url = this.fetch_urls.pop();
       $.ajax({
            ...
            { 
              ...
              //使用jQuery的选择器获取页面中需要的数据
              var a = $(data);
              var all_node = a.find('.reviews-list').children();
              var post_data = [];
              if (all_node.length) {
                  var l = all_node.length;
                  for (var n = 0 ;n < l; n++) {
                      var c_node = $(all_node[n]);
                      if (c_node.children()) {
                          // ignore hr
                          var f = $(all_node[n]);
                          if (f[0].localName == 'div') {
                              var tmp = {};
                              tmp.id = f.attr('data-reviewid');
                              tmp.title = f.find('h4')[0].innerText;
                              tmp.rank = f.find('.reviews-list-left .grid-parent .vertical-align-center .screen-reader-only')[0].innerText;
                              tmp.timeline = f.find('.reviews-list-left .grid-parent .padding-left-medium .darkMidGrey')[0].innerText;
                              tmp.text = f.find('.reviews-list-left .reviews-helpful-text')[0].innerText;
                              tmp.user = f.find('.reviews-list-right .user-name .user-first-name')[0].innerText;
                              post_data.push(tmp);
                          }
                      }
                  }
              ...
              $.post( this.posturl , { content: post_data , url : now_url } , ( data2 ) => 
              {
                this.flog = data2 + ' success';
              } );
            }
            ,error: (data) => 
            {
